...
const Item = ({ id, documentIcon, active, expanded, isSearch, level = 0, onExpand, label, onClick, icon: Icon }: ItemProps) => {
    ...
    const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        event.stopPropagation();
        if (!id) return;
        const promise = create({ title: "Untitled", parentDocument: id }).then((docmentId) => {
    ...
    return ( 
        <div 
            ...
            active && "bg-primary/5 text-primary"
                ...
                {documentIcon ? (
                        <div className="shink-0 text-[18px] mr-2">
                            {documentIcon}
                        </div>
                      ) : (
                    ...
                    <div className="ml-auto flex items-center gap-x-2">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent className="w-60" align="start" side="right" forceMount>
                                ...
                                <div className="text-xs text-muted-foreground capitalize p-2">
                                    Last Edited by: {user?.username}
                                </div>


                                <DropdownMenuItem onClick={() => router.push(`/documents/${id}/edit`)}>
                        </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                        <div 
