    ...
    handler: async(ctx, args) => {
        ...
        const recursiveArchive = async (documentId: Id<"documents">) => {
            const children = await ctx.db.query('documents').withIndex('by_user_parent', (q) =>
                q.eq('userId', userId).eq('parentDocument', documentId)).collect();
...
export const get_sidebar = query({
    args: {
...
export const getTrash = query({
    handler: async(ctx) => {
        ...
        const documents = await ctx.db.query('documents').withIndex('byUserId', (q) =>
            q.eq('userId', userId)
        ).filter((q) => 
